...
Public progress derives the PR4 handoff from canonical workflow state, not from the last completed display step. A workflow at `ready_for_search_intelligence` returns `state: research_ready`, `currentStep: research_ready`, no percentage, and no report. The visitor UI describes this as initial company research being complete while broader search intelligence is being prepared; it does not claim that search intelligence or the report has completed.
